...
int main(int argc, char**)
{
	...
	while (cap.isOpened())
	{
		...
		// Forward pass of the blob through the neural network to get predictions 
		...
		for (int i = 0; i < results.rows; i++) {
			...
			// Check if the detection is over the min threshold 
		...
		putText(image, "FPS: " + to_string(int(1 / totalTime)), Point(50, 50), FONT_HERSHEY_DUPLEX,1.0,Scalar(0,255,0),2);
